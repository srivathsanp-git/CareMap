    ...
    Promise.allSettled(jobs)
      .then(([cdc, census]) => {
        ...
        if (census && census.status === 'rejected') {
          console.info('[CareMap] Census ACS unavailable (poverty from snapshot):', census.reason?.message || census.reason)
        }
        if (!places && !acs) {
          console.warn('[CareMap] live data unavailable, using snapshot')
